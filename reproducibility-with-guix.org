...
Side note: the option =--container= requires support from the Linux kernel that is not available on all systems. By default, it is not turned on for unprivileged users so as =root= you need to run =echo 1 > /proc/sys/kernel/unprivileged_userns_clone=. If it doesn't work for you, use =--pure= instead. It provides a less isolated environment, but it is usually more than good enough.
...
: Inputs   : (gcc@8.3.0 ld-wrapper@0 binutils@2.31.1 glibc@2.28 glibc@2.28 glibc@2.28)
...
Last, if your colleague does not use yet Guix, then let pack (plain tarball, Docker or Singularity containers) and provide the image. For example,
#+begin_src sh :results none :exports code
guix pack            \
     -f docker       \
     -C none         \
